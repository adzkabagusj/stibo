"""
╔══════════════════════════════════════════════════════════════════════╗
║  STIBO INBOUND XML GENERATOR — NEW BALANCE (Sample / Apparel) v1.0   ║
...
Input folder : /tmp/stibo_workdir/input/sample_apparel/
...
THE RUNNING ALPHABET (BM rows 23/24/26 "1 Digit Running Alphabeth"):
    ...
    2nd → "B", … (>26 rows per base would wrap — a warning is logged; the
    live files peak at 11 rows per base). Both distinct 8-char styles that
    collide on a 7-char base (e.g. MT71W0NL / MT71W0NC) and the multiple
    colours of one style are disambiguated by this counter.

────────────────────────────────────────────────────────────────────────
COLUMN → ATTRIBUTE MAP (apparel sheet — every mapped column)
────────────────────────────────────────────────────────────────────────
  SKU                  → AT_PrincipalStyleCode   ("S"+SKU, raw)  [v6 R026]
                       → AT_Gender   (first SKU letter: W=F, M=M, Y=U) [R051]
  ...
  Product Line         → AT_PrincipalMerchandiseHierarchyL1          [R091]
                       → division parent (Apparel → PPH_A-…)
  Line Plan Business   → AT_PrincipalMerchandiseHierarchyL2          [R092]

  Apparel defaults (no source column — v6 "NEW BALANCE - SAMPLE"):
    SAP Gender  → from SKU first letter (v6 R051 App rule)          [R051]
...
────────────────────────────────────────────────────────────────────────
  ...
  Primary Fabric Content, Secondary Fabric Content, Supplier, Vendor Code,
  Factory Address Code, and ALL 44 country sample-quantity columns
  (AUSTRALIA … HONG KONG - REGIONAL) plus the trailing "PL" column.
  None of these appear as a "Field Name in the Brand File" for any attribute
  row in the two SAMPLE mapping sheets, so they are deliberately not emitted.
...
────────────────────────────────────────────────────────────────────────
 ...
 2. AT_BYArticleType is emitted as the constant ID="Inline" for every row
    ...
    "Channel Type" column exists (all rows read "Inline" in this file).
    The SAMPLE spec pins the default and does not read Channel Type.
...
SAMPLE_DIR   = INPUT_DIR / "sample_apparel"
...
log_path = LOG_DIR / f"run_nb_sample_apparel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
# ── v6 R051 App rule: SKU first letter → SAP Gender code ────────────────────
#   W : Female   M : Male   Y : Unisex   (anything else defaults to Unisex
#   and is logged as a validation warning)
...
DIVISION = "Apparel"   # fixed by file type → PPH parent letter "A"
...
class ApparelBreakoutLoader:
    """
    Loads the New Balance Global Inline Apparel Sample Breakout workbook.
    ...
    def _load(self):
        log.info("[Sample-APP] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)
        ...
        for target in targets:
            ...
            if hdr_idx is None:
                log.warning("[Sample-APP] No '%s' header row in sheet '%s' — skipping.",
                            self.ROW_KEY, target)
                continue

            log.info("[Sample-APP] Sheet '%s': header at row index %d (row %d)",
        ...
        if missing:
            log.warning("[Sample-APP] Expected source column(s) absent: %s", missing)

        total = sum(len(f) for f in self.frames)
        log.info("[Sample-APP] %d SKU row(s) loaded from sheet(s): %s", total, self.sheets)
...
def _sku_first_char_to_gender_code(sku: str) -> str:
    """v6 R051 App rule: first digit of SKU — W: Female, M: Male, Y: Unisex."""
    first = (sku or "").strip()[:1].upper()
    return SKU_FIRST_CHAR_TO_GENDER.get(first, "U")
...
def map_sku(row: pd.Series, brand_code: str = "NEW", mdd=None) -> dict:
    """Map one Apparel Sample Breakout row to a normalized sample-article dict."""
    sku             = _s(row.get("SKU"))
    ...
    # v6 R051 App: gender from the SKU's first letter; v6 R050: Adults default
...
def validate(mapped: dict, mdd: MDDLoader) -> list[str]:
    ...
    first = (mapped.get("sku") or "")[:1].upper()
    if first not in SKU_FIRST_CHAR_TO_GENDER:
        warns.append(
            f"[{sku}] SKU first letter '{first}' not in W/M/Y — AT_Gender defaulted to Unisex"
        )
...
def _get_division_code(division: str) -> str:
    ...
    for key, code in DIVISION_PARENT_MAP.items():
        if key in div_upper:
            return code
    return "A"   # this module's division default: Apparel
...
def _add_generic_values(vals_el, art, brand_name, comp_code, sbu, mdd=None, bm=None):
    ...
    populated for the apparel sample flow (see EMITTED_ATTRIBUTE_IDS).
    ...
    # ── Gender / age (App — from SKU first letter; Adults default) ─
    ...
    # ── Merchandise hierarchy (App — Product Line / Line Plan) ─────
...
def run(args, auditor=None):
    ...
    for sm_path in [sm_f]:
        log.info("─── Processing Apparel Sample Breakout file: %s ───", sm_path.name)

        loader = ApparelBreakoutLoader(sm_path, sheet_mode=SHEET_MODE)
        if not loader.frames:
            log.warning("[Sample-APP] No data frames loaded — skipping file.")
            continue

        total_rows = sum(len(f) for f in loader.frames)
        log.info("[Sample-APP] %d valid SKU row(s) to process", total_rows)
        ...
        if loader.running_letter_wraps:
            ...
            log.warning("[Sample-APP] %s", warn_msg)
        ...
        if dupes:
            log.warning(
                "[Sample-APP] %d duplicate KEY_InboundArticle value(s) — "
        ...
        if sap_dupes:
            log.warning(
                "[Sample-APP] %d duplicate AT_SAPStyleCode value(s) — "
        ...
        print(f"  Sample APP rows     : {total_rows}",    flush=True)
        print(f"  Mapped SKUs         : {written_count}", flush=True)
        print(f"  XML file size       : {file_kb}KB",     flush=True)
        print("════════════════════════════════════════════════════", flush=True)

    rpt_path = LOG_DIR / f"validation_nb_sample_apparel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
def main():
    p = argparse.ArgumentParser(
        description="Stibo Inbound XML Generator — New Balance Sample (Apparel) v1.0"
    )
