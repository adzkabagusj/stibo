"""
╔══════════════════════════════════════════════════════════════════╗
║   STIBO INBOUND XML GENERATOR — Steve Madden Handbag PO v1.0    ║
║   Handbag PO Form → Stibo STEP XML                              ║
║   (Generic Articles + Colour Variants + Size Variants)          ║
╚══════════════════════════════════════════════════════════════════╝

Input file pattern:
    0888-FF-STEVE MADDEN (SVM)-HANDBAG PO FORM (MAA Fashion Footwear) Inline-Multi-SP2029-ID-1
...
INPUT_DIR       = BASE_DIR / "input" / "handbag_po"
...
class RNALoader:
    ...
    Lookup key: (country_name, comp_code, sbu, brand_code) → brand_type, brand_status, brand_group
    """
...
class SteveMaddenFootwearPOLoader:
    ...
    # Signal words that identify the header row (normalised: lowercase, no spaces, no underscores)
    HEADER_SIGNALS = {"style", "style#", "colorcode", "customerprice", "barcode", "pono", "stylename"}
    ...
    def _load(self):
        print(f"[_load] Starting load for: {self.path.name}", flush=True)
        print(f"[_load] HEADER_SIGNALS = {self.HEADER_SIGNALS}", flush=True)
        ...
        for i in range(min(20, len(rows))):
            if rows[i]:
                ...
                if i < 5:  # Print first 5 rows for debugging
                    print(f"[_load] Row {i} normalized: {rv}", flush=True)
                if rv & self.HEADER_SIGNALS:
                    hdr_idx = i
                    print(f"[_load] ✓ HEADER FOUND at row {hdr_idx}", flush=True)
                    break
        ...
        style_col = self._find_col(header, ["STYLE #", "Style #", "STYLE#", "Style#", "STYLE", "Style", "style"])
        if style_col:
            df = df[
                df[style_col].notna()
        ...
        print(f"[Loader] Loaded {len(self.df)} data rows", flush=True)
        print(f"[Loader] All headers: {header}", flush=True)
        log.info(
...
def group_rows(
    ...
    """
    col_style       = loader.find_col("STYLE #", "Style #", "STYLE#", "Style#", "STYLE", "Style")
    col_style_num   = loader.find_col("STYLE #", "Style #", "STYLE#", "Style#")
    col_style_name  = loader.find_col("STYLE NAME", "Style Name", "STYLENAME")
    col_color_code  = loader.find_col("COLOR CODE", "Color Code", "COLOR_CODE", "COLORCODE")
    col_color_desc  = loader.find_col("COLOR CODE DESCRIPTION", "Color Description", "COLOR CODE DESC")
    col_price       = loader.find_col("CUSTOMER PRICE", "Customer Price", "CUSTOMERPRICE", "PRICE", "Price", "FOB")
    col_category    = loader.find_col("PRODUCT CATEGORY", "Category", "PRODUCTCATEGORY")
    col_barcode     = loader.find_col("BARCODES", "Barcode", "EAN")
    col_coo         = loader.find_col("COO", "Country of Origin", "CountryOfOrigin", "COUNTRY OF ORIGIN")

    # ── Diagnostic printing ──────────────────────────────────────
    print(f"[Grouper] col_style={col_style}  col_style_num={col_style_num}  col_style_name={col_style_name}", flush=True)
    print(f"[Grouper] col_color_code={col_color_code}  col_color_desc={col_color_desc}  col_price={col_price}", flush=True)
    print(f"[Grouper] col_category={col_category}  col_barcode={col_barcode}  col_coo={col_coo}", flush=True)
    print(f"[Grouper] Total raw_rows: {len(raw_rows)}", flush=True)
    if raw_rows:
        print(f"[Grouper] Sample row keys: {list(raw_rows[0].keys())[:30]}", flush=True)
        print(f"[Grouper] Sample row[0] values: { {k: raw_rows[0][k] for k in list(raw_rows[0].keys())[:10]} }", flush=True)
    ...
    for row in raw_rows:
        ...
        if generic_key not in result:
            style_name = _s(row.get(col_style_name) if col_style_name else "") or style_code
            result[generic_key] = {
                "generic_key": generic_key,
                "style_code": style_code,
                "AT_InboundGenericCode": generic_key,
                "AT_PrincipalStyleCode": _s(row.get(col_style_num) if col_style_num else "") or style_code,
                "AT_PrincipalColorCode": color_code,
                "AT_PrincipalColorName": color_desc,
                "AT_PrincipalStyleDescription": style_name,
                "AT_Collection1": style_name,
    ...
    print(
        f"[Grouper] {len(raw_rows)} rows → {len(result)} generics | "
        f"skipped: no_style={skipped_no_style}  no_color={skipped_no_color}",
        flush=True,
    )
    log.info(
        "[Grouper] %d rows → %d generics, %d total variants | skipped: no_style=%d  no_color=%d",
        len(raw_rows),
        len(result),
...
def build_product_xml(
    ...
    g_el.set("ParentID",   "PPH_E-TempSubCat")  # Accessories/Handbag hierarchy
    ...
    _val_lov(gv, "AT_PackDetails",        "S")     # Default pack details
    _val_lov(gv, "AT_EComAgesCategory",   "18+Y")  # E-commerce age category
    _val_lov(gv, "AT_BYArticleType",      "Inline")  # Article Type
...
def run(args, auditor=None):
    ...
    """
    print("=== STEVE MADDEN HANDBAG PO v2.1 (2026-08-11 HEADER FIX) ===", flush=True)
    ...
    if not input_file:
        ...
        return
    print(f"[run] Input file found: {input_file.name}", flush=True)
    ...
    # ── RNA lookup: brand_type, brand_status ─────
    brand_type = brand_status = brand_group = ""
    if rna:
    ...
    print(f"[run] Calling group_rows with {len(raw_rows)} rows, brand_code={brand_code}", flush=True)
    generics = group_rows(raw_rows, loader, brand_code, mdd)
    print(f"[run] group_rows returned {len(generics)} generics", flush=True)
