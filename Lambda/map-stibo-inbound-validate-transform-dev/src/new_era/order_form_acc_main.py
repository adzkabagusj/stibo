"""
╔══════════════════════════════════════════════════════════════════╗
║   STIBO INBOUND XML GENERATOR — New Era Order Form Acc v1.0      ║
║   Fully mapping-driven: no per-attribute Python code              ║
╚══════════════════════════════════════════════════════════════════╝

Input file : "...Order form - APAC FW26 (ACC)..." (sheet "FW26 APAC ACC")
...
of attributes to emit from these columns:
  ...
  G  Field Mapping Type         — decides whether the attribute is included:
                                     ...
                                     otherwise                          -> excluded
  J  Field Name in the Brand File — column to read on the "FW26 APAC ACC"
...
convention as clarks/order_form_accs_main.py, steve_madden/handbag_po_main.py,
implus's accessories modules). Change DEFAULT_PRODUCT_PARENT_ID if New Era
Order Form Acc articles route to a different division.
...
INPUT_DIR   = BASE_DIR / "input" / "orderform_acc"
...
INPUT_SHEET_NAME   = "FW26 APAC ACC"     # sheet in the New Era input file
...
#     File (ACC) Headwear
#     Column : MI
# "FW26 APAC ACC" (this file) is confirmed to be the "Bags and Other
# Accessories" file specifically — TARGET_FILE_TYPE_LABEL below is matched
# case-insensitively as a substring against each block's "File (...) LABEL"
# line to pick out just that file type's column name. Col J values with no
# "File (...)" blocks at all (plain single-value cells, e.g. "EAN/UPC") are
# used as-is.
TARGET_FILE_TYPE_LABEL = "Bags and Other Accessories"
...
#   AT_Material: "Fabric Content" column's highest-percentage material
#   (e.g. "100% POLYESTER RIPSTOP 200D" -> "POLYESTER RIPSTOP 200D"),
#   matched against the MDD "Material LOV" tab (Col B display name
#   substring-matched -> Col A LOV id).
...
#   Accessories is "Column : O - AN (No Fill Color)": columns O..AN of the
...
SIZE_GRID_FIRST_COL = "L"
SIZE_GRID_LAST_COL  = "AG"
...
def _read_size_grid_fills(path: Path, sheet_name: str, header_row_idx: int,
    ...
    if not size_labels:
        log.warning(
            "[NewEra-OrderFormAcc] Size grid columns %s:%s have no header labels — "
...
# ======================================================================
# INPUT FILE LOADER  ("FW26 APAC ACC" sheet — header row/columns
...
class NewEraOrderFormLoader:
    ...
    def _load(self) -> None:
        log.info("[NewEra-OrderFormAcc] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)
        target = next(
            (s for s in wb.sheetnames if s.strip().upper() == self.sheet_name.upper()),
            None,
        ...
        if not target:
            wb.close()
            raise ValueError(
                f"[NewEra-OrderFormAcc] Sheet '{self.sheet_name}' not found. "
                f"Available: {wb.sheetnames}"
            )
        log.info("[NewEra-OrderFormAcc] Using sheet: '%s'", target)

        raw_rows = list(wb[target].iter_rows(values_only=True))
        wb.close()

        if not raw_rows:
            log.warning("[NewEra-OrderFormAcc] Sheet '%s' is empty", target)
            return
        ...
        if best_score <= 0:
            log.warning(
                "[NewEra-OrderFormAcc] Could not confidently locate header row "
                "(no cell matched any expected column name) — defaulting to row 0"
            )
        log.info(
            "[NewEra-OrderFormAcc] Header at row %d (0-indexed), matched %d/%d expected columns",
        ...
        log.info("[NewEra-OrderFormAcc] Loaded %d data rows", len(self.rows))
...
def build_generic_product(
    ...
    if price_col:
        ...
        if raw_price:
            try:
                ...
                log.info(
                    "[XML] AT_FOB: raw Price value '%s' is not numeric — attribute skipped for %s",
    ...
    # AT_Material: "Fabric Content" column's highest-percentage material,
    # matched against the MDD "Material LOV" tab (per user direction,
    # 2026-08-19). No fallback — skipped if no match is found.
...
def run(args, auditor=None):
    ...
    log.info("[NewEra-OrderFormAcc] Starting: brand=%s code=%s season=%s", brand, brand_code, season)
    ...
    if mdd_file:
        ...
        log.warning("[NewEra-OrderFormAcc] No MDD file found in %s — LOV lookups disabled", MDD_DIR)
    ...
    if not attr_map.rules:
        log.warning("[NewEra-OrderFormAcc] No active attribute rules found — nothing will be sent")
    ...
    if mdd:
        ...
        log.info(
            "[NewEra-OrderFormAcc] Brand LOV lookup: key='%s' -> LOV ID='%s' %s",
    ...
    # ── Load the New Era Order Form Acc input file ─────────────────────
    input_file = next(INPUT_DIR.glob("*.xls*"), None)
    if not input_file:
        raise FileNotFoundError(f"No New Era Order Form Acc file found in {INPUT_DIR}")
    ...
    if not loader.rows:
        log.warning("[NewEra-OrderFormAcc] No data rows found — nothing to process")
        return
    ...
    if not style_col:
        raise ValueError(
            f"[NewEra-OrderFormAcc] Could not find column 'MI#' "
    ...
    if not generics:
        log.warning("[NewEra-OrderFormAcc] No valid generics produced")
        return
    ...
    log.info("[NewEra-OrderFormAcc] Writing XML -> %s", xml_filename)
    ...
    log.info("[NewEra-OrderFormAcc] XML written: %s (%d KB)", xml_path.name, file_kb)
    print(
        f"=== NEW ERA ORDER FORM ACC SUMMARY ===\n"
...
def _parse_metadata_from_filename(dirs: dict, triggered_file_type: str = None) -> dict:
    ...
    e.g. "0888-SP-NEW ERA-Copy of Order form - APAC FW26 (ACC)_Revised Size
    ...
    candidate_dirs.append(dirs.get("orderform_acc"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="New Era Order Form Acc -> Stibo XML")
