"""
╔══════════════════════════════════════════════════════════════════╗
║   STIBO INBOUND XML GENERATOR — Reebok Footwear v1.0            ║
║   Article Master (NuORDER export) → Stibo STEP XML              ║
║   (Generic Articles only — variant explosion tracked for audit) ║
╚══════════════════════════════════════════════════════════════════╝

Source file
    "Reebok article master data with image - FOOTWEAR (SS27).xlsx"
...
The FOOTWEAR export shares the exact same "NuORDER Order Data" column
layout as the APPAREL export (including the "Category" / "Subcategory"
shoe-hierarchy columns and per-standard Size_US/UK/EUR/... columns).
Only the "Category"/"Subcategory" columns are footwear-only in intent —
they are left unmapped here for the same reason as the shoe-specific
attributes noted below (no confirmed Stibo attribute target yet); extend
`group_rows()` / `build_product_xml()` once the mapping is confirmed.
...
and Silhouette/Fabric/Upper Material) are intentionally left out of the
XML rather than guessed — extend `build_product_xml()` once those lookup
tables exist.
...
INPUT_DIR   = BASE_DIR / "input" / "footwear"
...
# Age Group (Reebok file, e.g. "Adult", "Kids", "Junior", "Infant", "CHILDRENS")
# → (AT_SAPAge, AT_BYAge) codes. NOT sourced from the MDD "Age LOV" sheet —
# those columns don't hold valid live-Stibo LOV IDs. Confirmed against the
# brand mapping template's own "Reebok MD Mappings sheet 1" tab (Reebok raw
# Age Group → Stibo codes: Adult→AD-Adults, Infant→CH-Children/Infant,
# CHILDRENS→CH-Children/Kids, Junior→CH-Children/Grade School, Kids→CH-
# Children/Kids), cross-checked against nike/orderConfirmation_main.py
# (NIKE_AGE_DESC_RULES, same MDD, confirmed against live Stibo): AT_SAPAge
# (LOV_Age) only accepts AD / CH / AA in all supported environments;
# AT_BYAge (LOV_BYAge) only accepts ADULT / ALL AGES / GRADE SCHOOL /
# INFANT / KIDS / PRESCHOOL.
...
# Raw "Article Status" text (Reebok file) → AT_ArticleStatus LOV code
ARTICLE_STATUS_MAP: dict[str, str] = {
    "BUY READY":    "A",
    "ACTIVE":       "A",
    "CONFIRMED":    "A",
    "DISCONTINUED": "D",
...
# Raw "Subcategory" text (Reebok footwear file) → AT_Silhouette LOV code.
# Per the brand mapping template's "Reebok MD Mappings sheet 1" tab (Reebok
# raw Subcategory → Stibo Silhouette label), resolved against the MDD
# "Silhouette LOV" sheet's numeric codes. The Reebok source file itself
# truncates some values (e.g. "SHOES - HIGH (NO-FB" with no closing paren,
# "FOOTBALL SHOES (FIRM") — the keys below match those truncated forms
# exactly, as seen in the real file. Apparel Subcategory values (T-SHIRTS,
# PANTS, etc.) have no Silhouette mapping in the template — intentionally
# absent here.
SUBCATEGORY_SILHOUETTE_MAP: dict[str, str] = {
    "SHOES - LOW (NO-FB)":  "20",  # Low Cut Shoes
    "SHOES - LOW":           "20",  # Low Cut Shoes
    "SHOES - MID (NO-FB)":  "23",  # Mid Cut Shoes
    "SHOES - HIGH (NO-FB":  "15",  # High Cut Shoes
    "SHOES - HIGH (NO-FB)": "15",  # High Cut Shoes
    "SLIDES":                "42",  # Slides
    "SANDALS":                "42",  # Slides
    "FOOTBALL SHOES (FIRM":  "37",  # Shoes
    "FOOTBALL SHOES (ARTI":  "37",  # Shoes
    "FOOTBALL SHOE-SOFT-G":  "37",  # Shoes
    "FOOTBALL SHOES (INDO":  "37",  # Shoes
    "FOOTBALL SHOES (TURF":  "37",  # Shoes
}
...
def _top_material(material_composition: str) -> str:
    ...
    Footwear cells look like:
      "Upper Composition: 63.8% TEXTILE - POLYESTER, 36.2% PU - COATED
       SYNTHETIC Bottom Unit Composition: 100% Outsole-RUB Flocking: ..."
    — only the "Upper Composition" segment is considered there. Apparel
    cells commonly stack multiple labeled sections separated by "/" (e.g.
    "Shell: 87% PA, 13% EL / Panel: 92% PES, 8% EL"), sometimes with no
    delimiter at all ("60% Cotton 40% Polyester"). The name for each entry
    is captured up to the next "%", "," or "/" so entries don't swallow
    their neighbors; a stray trailing number (from a delimiter-less
...
class ArticleMasterLoader:
    """
    Loads the Reebok Footwear Article Master (NuORDER export) from Excel.
    """
    ...
    def _load(self):
        log.info("[FootwearArticleMaster] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)
        log.info("[FootwearArticleMaster] Available sheets: %s", wb.sheetnames)

        target = next(
            (s for s in wb.sheetnames if s.strip() in self.PREFERRED_SHEETS),
            wb.sheetnames[0],
        )
        log.info("[FootwearArticleMaster] Using sheet: '%s'", target)
        ...
        if not rows:
            log.warning("[FootwearArticleMaster] Sheet '%s' is empty", target)
            return
        ...
        for i in range(min(20, len(rows))):
            if rows[i] and any(
                ...
                hdr_idx = i
                log.info("[FootwearArticleMaster] Header detected at row %d (found '%s')", hdr_idx, self.HEADER_SIGNAL)
                break
        else:
            log.warning(
                "[FootwearArticleMaster] '%s' not found in first 20 rows — defaulting to row 0",
                self.HEADER_SIGNAL,
            )
        ...
        log.info(
            "[FootwearArticleMaster] Loaded %d data rows | %d columns",
...
def group_rows(
    ...
    col_subcat   = loader.find_col("Subcategory")
    ...
    log.info(
        ...
        "upc=%s coo=%s dept=%s sports=%s segment=%s mkt_line=%s subcat=%s material=%s status=%s "
        "fob=%s msrp=%s launch=%s uom=%s",
        col_article, col_name, col_color, col_gender, col_age, col_size,
        col_upc, col_coo, col_dept, col_sports, col_segment, col_mkt_line, col_subcat, col_material,
    ...
    for row in raw_rows:
        ...
        if generic_key not in result:
            result[generic_key] = {
                ...
                "subcategory":      _s(row.get(col_subcat, "")) if col_subcat else "",
...
def build_product_xml(
    ...
    # ── Article Status (raw text → LOV code, best-effort) ──────────
    if generic.get("status_raw"):
        status_code = ARTICLE_STATUS_MAP.get(generic["status_raw"].upper())
        if status_code:
            _val_lov(gv, "AT_ArticleStatus", status_code)
        else:
            log.warning(
                "[XML] Unmapped Article Status '%s' for %s — defaulting to 'A'",
                generic["status_raw"], generic["generic_key"],
            )
    ...
    # ── Silhouette (Subcategory → LOV — see SUBCATEGORY_SILHOUETTE_MAP) ──
    if generic.get("subcategory"):
        silhouette_code = SUBCATEGORY_SILHOUETTE_MAP.get(generic["subcategory"].upper())
        if silhouette_code:
            _val_lov(gv, "AT_Silhouette", silhouette_code)
        else:
            log.warning(
                "[XML] Subcategory '%s' not found in SUBCATEGORY_SILHOUETTE_MAP — "
                "AT_Silhouette skipped for %s",
                generic["subcategory"], generic["generic_key"],
            )
...
def run(args, auditor=None):
    """Main entry point for Reebok Footwear Article Master ETL."""
    ...
    log.info(
        "[ReebokFootwear] Starting: brand=%s code=%s season=%s",
    ...
    if mdd_file:
        ...
        log.warning("[ReebokFootwear] No MDD file found in %s — LOV lookups disabled", MDD_DIR)

    rna = None
    mapping_file = next(ATTR_DIR.glob("*.xlsx"), None) or next(ATTR_DIR.glob("*.xlsm"), None)
    if mapping_file:
        rna = RNALoader(mapping_file)
        log.info("[ReebokFootwear] RNA loaded: %d entries", len(rna.lookup))
    else:
        log.warning("[ReebokFootwear] No attributes/brand-mapping file found in %s — RNA disabled", ATTR_DIR)

    input_file = next(INPUT_DIR.glob("*.xls*"), None)
    if not input_file:
        raise FileNotFoundError(f"No Reebok footwear article master file found in {INPUT_DIR}")
    ...
    if country_code and mdd:
        ...
        if not rna_country:
            log.warning("[ReebokFootwear] Country ID '%s' not found in Country LOV", country_code)
    if not rna_country:
    ...
    if mdd:
        ...
        if looked_up:
            ...
            log.warning(
                "[ReebokFootwear] '%s' not in Brand LOV — using fallback brand_code='%s'",
    ...
    if loader.df.empty:
        log.warning("[ReebokFootwear] No data rows found — nothing to process")
        return None
    ...
    log.info("[ReebokFootwear] Writing XML → %s", xml_filename)
    ...
    log.info("[ReebokFootwear] XML written: %s (%d KB)", xml_path.name, file_kb)
    print(
        f"=== REEBOK FOOTWEAR SUMMARY ===\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reebok Footwear Article Master → Stibo XML")
