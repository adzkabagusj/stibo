"""
╔══════════════════════════════════════════════════════════════════╗
║      STIBO INBOUND XML GENERATOR — ASTEC Licensed Recap v1.1   ║
║  Licensed Recap Sample Development → Stibo STEP XML            ║
╚══════════════════════════════════════════════════════════════════╝

ASTEC Licensed Recap-specific workflow (similar to New Balance Licensed):
...
LOV_BRAND = {
    ...
    "LOT": "LOTTO",   "BIR": "BIRKENSTOCK", "AIW": "AIRWALK", "ASC": "ASTEC", 
}
...
ASTEC_GENDER_TO_SAP: dict[str, str] = {
    "MALE":   "M",
    "FEMALE": "F",
    "UNISEX": "U",
    "BOY":    "M",
...
ASTEC_GENDER_TO_AGE: dict[str, str] = {
    "MALE":   "AD",   # Adults
    "FEMALE": "AD",
    "UNISEX": "AD",
    "MEN":    "AD",
...
ASTEC_AGE_GROUP_MAP: dict[str, tuple[str, str]] = {
...
ASTEC_GENDER_TO_AGE_GROUP: dict[str, str] = {
...
ASTEC_CATEGORY_TO_DIVISION: dict[str, str] = {
    "OUTDOOR":      "F",   # Footwear
    "CASUAL":       "F",
    "KIDS":         "F",
    "RUNNING":      "F",
...
ASTEC_ARTICLE_TYPE_CODE: dict[str, str] = {
    "LICENSE":   "R",
    "SSE":       "X",
    "WHOLESALE": "W",
    "SAMPLE":    "S",
...
ASTEC_SIZE_TO_SAP: dict[str, str] = {
    "36":   "036",
    "37":   "037",
    "38":   "038",
    "39":   "039",
...
ASTEC_MATERIAL_TYPE_LOV: dict[str, str] = {
...
class AttributesListLoader:
    ...
    def __init__(self, path: Path, brand: str = "ASTEC"):
        self.path      = path
    ...
    def _load(self):
        ...
        sheet_name = "Astec(Inline + Licensed)"
        if sheet_name not in wb.sheetnames:
...
class AstecRecapLoader:
    ...
    def _load(self):
        log.info("[Recap-ASTEC] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)

        # Use LOT or LOTTO sheet
        if "ASC" in wb.sheetnames:
            target = "ASC"
        elif "ASTEC" in wb.sheetnames:
            target = "ASTEC"
        else:
            target = wb.sheetnames[0]
        log.info("[Recap-ASTEC] Using sheet: '%s'", target)
        ...
        for i, row in enumerate(rows[:10]):  # Check first 10 rows
            ...
            if any(kw in row_str for kw in ["SUPP ART", "COLOR", "GENDER", "CATEGORY", "SIZE RANGE"]):
                hdr_idx = i
                log.info("[Recap-ASTEC] Found header at row %d (0-indexed, row %d in Excel)", hdr_idx, hdr_idx + 1)
                break
        
        if hdr_idx is None:
            log.warning("[Recap-ASTEC] Could not find header row, defaulting to row 3 (index 2)")
        ...
        log.info("[Recap-ASTEC] Header row %d: %s", hdr_idx, header)
        
        df = pd.DataFrame(rows[hdr_idx + 1:], columns=header)
        
        log.info("[Recap-ASTEC] Found columns: %s", list(df.columns))
        
        # Log first data row for debugging
        if len(df) > 0:
            first_row_dict = df.iloc[0].to_dict()
            log.info("[Recap-ASTEC] First data row sample: %s", {k: v for k, v in list(first_row_dict.items())[:5]})
        ...
        if supp_art_col:
            log.info("[Recap-ASTEC] Found supplier article column: '%s'", supp_art_col)
            
            # Log sample values before filtering
            sample_values = df[supp_art_col].head(3).tolist()
            log.info("[Recap-ASTEC] Sample '%s' values before filtering: %s", supp_art_col, sample_values)
            ...
            if len(df) > 0:
                ...
                log.info("[Recap-ASTEC] Sample 'Supp Art #' values after filtering: %s", sample_after)
        else:
            log.warning("[Recap-ASTEC] 'Supp Art #' column not found! Available columns: %s", list(df.columns))

        wb.close()
        self.df    = df.reset_index(drop=True)
        self.sheet = target
        log.info("[Recap-ASTEC] %d rows loaded", len(self.df))
...
def _mdd_lov_id(mdd, lov_names, display: str, id_first: bool = False) -> str:
    ...
    col A = display, col B = id.  "Brand Group LOV" is the other way round
    (col A = id, col B = display — see the Pazzion loader), so callers pass
    ``id_first=True`` for it.  A value that already is an id comes back as is.
    """
...
def _size_to_sap_code(size_raw: str) -> str:
    ...
    if key in ASTEC_SIZE_TO_SAP:
        return ASTEC_SIZE_TO_SAP[key]
...
def map_article_astec(recap_row: dict, brand_code: str = "ASC") -> dict:
    """
    ...
    # "Mapping to STIBO" row 147: the recap header is "FOB Currency" (1st & 2nd
    ...
    # "Mapping to STIBO" row 219: BCI is Manual Input on the 1st ingestion and
    ...
    # ── Derived fields ──────────────────────────────────────────
    # SAP Gender
    gender_code = ASTEC_GENDER_TO_SAP.get(gender.upper(), "U")

    # SAP Age
    age_code = ASTEC_GENDER_TO_AGE.get(gender.upper(), "AD")

    # Division: prefer Code Category, fallback to Category
    div_source = code_category if code_category else category
    div_letter = ASTEC_CATEGORY_TO_DIVISION.get(div_source.upper(), "F")
    ...
    # Article Type (1 char)
    art_type_char = ASTEC_ARTICLE_TYPE_CODE.get(article_type_raw.upper(), "R")
...
def _add_generic_values(
    ...
    # AT_BrandGroup — "Mapping to STIBO" row 27: Formula in System, taken from
    # the Attributes List sheet "2. Source Mapping related RNA", column
    # BRANDGROUP, keyed on Compcode + SBU (+ brand).  The MDD "Brand Group LOV"
    # turns the display value into the LOV id.  Falls back to the brand name
    # only when the RNA sheet has no row for this brand.
    ...
    # BCI — "Mapping to STIBO" row 219: 1st ingestion = Manual Input (filled
    ...
    # ── Material Type ────────────────────────────────────────────
    _w("AT_MaterialType", "", id_val=ASTEC_DEFAULT_MATERIAL_TYPE)
...
def _resolve_gender_age(mapped: dict, md_mapping=None) -> None:
    ...
    sap_g, by_g = ASTEC_GENDER_MAP.get(_hkey(gender_raw), ("", ""))
...
def _process_article(row_tuple):
    ...
    mapped = map_article_astec(row, brand_code=brand_code)
...
def run(args, auditor=None):
    ...
    def find_brand_mapping_file(d: Path) -> Path | None:
        ...
        # AT_BrandGroup (Mapping to STIBO row 27), so it must be recognised too.
    ...
    md_map = AstecMDMappingLoader(attr_f)
    ...
    for recap_path in recap_files:
        ...
        recap = AstecRecapLoader(recap_path)
        if recap.df.empty:
            log.warning("[Recap-ASTEC] Empty dataframe — skipping."); continue
        ...
        log.info("[Recap-ASTEC] Country code resolved: '%s' for file: %s", file_country_code, recap_path.name)
        ...
        print("═══ ARTICLE SUMMARY (ASTEC LICENSED RECAP) ═════", flush=True)
...
def main():
    p = argparse.ArgumentParser(description="Stibo Inbound XML Generator — ASTEC Licensed Recap v1.0")
    p.add_argument("--brand",      default="ASTEC")
    p.add_argument("--brand-code", default="ASC")
