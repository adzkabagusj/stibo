"""
╔══════════════════════════════════════════════════════════════════╗
║      STIBO INBOUND XML GENERATOR — ELLESSE Licensed Recap v1.1   ║
║  Licensed Recap Sample Development → Stibo STEP XML            ║
╚══════════════════════════════════════════════════════════════════╝

ELLESSE Licensed Recap-specific workflow (similar to New Balance Licensed):

Source file
  • Single input file  : "RECAP SAMPLE DEVELOPMENT-LOT-{SEASON}-FOOTWEAR.xlsx"
  • Active sheet       : "ELLESSE"
...
LOV_BRAND = {
    ...
    "LOT": "LOTTO",   "BIR": "BIRKENSTOCK", "AIW": "AIRWALK", "ASC": "ASTEC", "ELL": "ELLESSE", 
}
...
ELLESSE_GENDER_TO_SAP: dict[str, str] = {
    "MALE":   "M",
    "FEMALE": "F",
    "UNISEX": "U",
    "BOY":    "M",
...
ELLESSE_GENDER_TO_AGE: dict[str, str] = {
    "MALE":   "AD",   # Adults
    "FEMALE": "AD",
    "UNISEX": "AD",
    "MEN":    "AD",
...
ELLESSE_CATEGORY_TO_DIVISION: dict[str, str] = {
    "OUTDOOR":      "F",   # Footwear
    "CASUAL":       "F",
    "KIDS":         "F",
    "RUNNING":      "F",
...
ELLESSE_ARTICLE_TYPE_CODE: dict[str, str] = {
    "LICENSE":   "R",
    "SSE":       "X",
    "WHOLESALE": "W",
    "SAMPLE":    "S",
...
ELLESSE_SIZE_TO_SAP: dict[str, str] = {
    "36":   "036",
    "37":   "037",
    "38":   "038",
    "39":   "039",
...
ELLESSE_MATERIAL_TYPE_LOV: dict[str, str] = {
...
ELLESSE_DEFAULT_MATERIAL_TYPE = "ZINA"
...
class AttributesListLoader:
    ...
    def __init__(self, path: Path, brand: str = "ELLESSE"):
        self.path      = path
    ...
    def _load(self):
        ...
        sheet_name = "Ellesse(Inline + Licensed)"
        if sheet_name not in wb.sheetnames:
...
class EllesseRecapLoader:
    ...
    def _load(self):
        log.info("[Recap-ELLESSE] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)

        # Use LOT or LOTTO sheet
        if "ELL" in wb.sheetnames:
            target = "ELL"
        elif "ELLESSE" in wb.sheetnames:
            target = "ELLESSE"
        else:
            target = wb.sheetnames[0]
        log.info("[Recap-ELLESSE] Using sheet: '%s'", target)
        ...
        for i, row in enumerate(rows[:10]):  # Check first 10 rows
            ...
            if any(kw in row_str for kw in ["SUPP ART", "COLOR", "GENDER", "CATEGORY", "SIZE RANGE"]):
                hdr_idx = i
                log.info("[Recap-ELLESSE] Found header at row %d (0-indexed, row %d in Excel)", hdr_idx, hdr_idx + 1)
                break
        
        if hdr_idx is None:
            log.warning("[Recap-ELLESSE] Could not find header row, defaulting to row 3 (index 2)")
        ...
        log.info("[Recap-ELLESSE] Header row %d: %s", hdr_idx, header)
        
        df = pd.DataFrame(rows[hdr_idx + 1:], columns=header)
        
        log.info("[Recap-ELLESSE] Found columns: %s", list(df.columns))
        
        # Log first data row for debugging
        if len(df) > 0:
            first_row_dict = df.iloc[0].to_dict()
            log.info("[Recap-ELLESSE] First data row sample: %s", {k: v for k, v in list(first_row_dict.items())[:5]})
        ...
        if supp_art_col:
            log.info("[Recap-ELLESSE] Found supplier article column: '%s'", supp_art_col)
            
            # Log sample values before filtering
            sample_values = df[supp_art_col].head(3).tolist()
            log.info("[Recap-ELLESSE] Sample '%s' values before filtering: %s", supp_art_col, sample_values)
            ...
            if len(df) > 0:
                ...
                log.info("[Recap-ELLESSE] Sample 'Supp Art #' values after filtering: %s", sample_after)
        else:
            log.warning("[Recap-ELLESSE] 'Supp Art #' column not found! Available columns: %s", list(df.columns))

        wb.close()
        self.df    = df.reset_index(drop=True)
        self.sheet = target
        log.info("[Recap-ELLESSE] %d rows loaded", len(self.df))
...
def _size_to_sap_code(size_raw: str) -> str:
    ...
    if key in ELLESSE_SIZE_TO_SAP:
        return ELLESSE_SIZE_TO_SAP[key]
...
def map_article_ellesse(recap_row: dict, brand_code: str = "ELL") -> dict:
    """
    ...
    # ── Derived fields ──────────────────────────────────────────
    # SAP Gender
    gender_code = ELLESSE_GENDER_TO_SAP.get(gender.upper(), "U")

    # SAP Age
    age_code = ELLESSE_GENDER_TO_AGE.get(gender.upper(), "AD")

    # Division: prefer Code Category, fallback to Category
    div_source = code_category if code_category else category
    div_letter = ELLESSE_CATEGORY_TO_DIVISION.get(div_source.upper(), "F")
    ...
    model_name = f"ELLESSE {supp_art} {gender.upper()} {color.upper()}".strip()
    ...
    # Article Type (1 char)
    art_type_char = ELLESSE_ARTICLE_TYPE_CODE.get(article_type_raw.upper(), "R")
...
def _add_generic_values(
    ...
    # ── Material Type ────────────────────────────────────────────
    _w("AT_MaterialType", "", id_val=ELLESSE_DEFAULT_MATERIAL_TYPE)
...
def run(args, auditor=None):
    ...
    md_map = EllesseMDMappingLoader(attr_f)
    ...
    for recap_path in recap_files:
        ...
        recap = EllesseRecapLoader(recap_path)
        if recap.df.empty:
            log.warning("[Recap-ELLESSE] Empty dataframe — skipping."); continue
        ...
        log.info("[Recap-ELLESSE] Country code resolved: '%s' for file: %s", file_country_code, recap_path.name)
        ...
        print("═══ ARTICLE SUMMARY (ELLESSE LICENSED RECAP) ═════", flush=True)
...
def main():
    p = argparse.ArgumentParser(description="Stibo Inbound XML Generator — ELLESSE Licensed Recap v1.0")
    p.add_argument("--brand",      default="ELLESSE")
    p.add_argument("--brand-code", default="ELL")
