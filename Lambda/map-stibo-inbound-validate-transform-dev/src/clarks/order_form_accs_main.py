"""
╔══════════════════════════════════════════════════════════════════╗
║   STIBO INBOUND XML GENERATOR — Clarks Order Form Accs v1.0     ║
║   Order Form Accs (Accessories) → Stibo STEP XML                ║
║   (Generic Articles + Colour Variants, No Size)                  ║
╚══════════════════════════════════════════════════════════════════╝

Input file pattern:
    RAW Order Form Clarks Accs SS27.xlsx

Sheet:
    "Order Form"  — header row at row 2 (0-indexed row 1)

Column layout (Clarks Order Form Accs — brand mapping sheet "Clarks(Inline)"):
...
INPUT_DIR       = BASE_DIR / "input" / "order_form_accs"
...
# ======================================================================
_GENDER_TO_SAP: dict[str, str] = {    # Accessories
    "WOMENS":             "Female",
    "MENS":               "Male",
    "BOYS":               "Male",
    "GIRLS":              "Female",
...
# ======================================================================
# Sock product values (exact match)
_SOCK_PROGRAM_VALUES = {
    "3pk sock liner",
    "3pk crew sock",
    "3pk sport ankle sock",
    "crew sock"
}

_CARE_SOCK = (
    "Machine wash with similar colors. "
    "Wash inside out to reduce pilling and preserve fabric quality. "
    "Use cold or warm water and a mild detergent. "
    "Replace socks showing signs of excessive wear to ensure optimal comfort and hygiene."
)

_CARE_SHOE_CARE = (
    "Follow product-specific instructions for application.. "
...
class MDDLoader:
    """
    Loads LOV tables from the MDD Excel needed for Clarks Accs.
    ...
    def _load_collection1_lov(self, wb):
        """Collection1 LOV: Load from 'Clarks MD Mapping' sheet, Col H (program) -> Col I (Collection1 LOV)."""
        sheet_name = next(
            (s for s in wb.sheetnames if "CLARKS" in s.upper() and "MD" in s.upper() and "MAPPING" in s.upper()),
            next((s for s in wb.sheetnames if "MD MAPPING" in s.upper()), None),
        )
        if not sheet_name:
            log.warning("[MDD] 'Clarks MD Mapping' sheet not found")
            return
        rows = list(wb[sheet_name].iter_rows(values_only=True))
        ...
        for row in rows[hdr_idx + 1:]:
            if not row or len(row) < 9:  # Need at least columns H and I (indices 7 and 8)
                continue
    ...
    def _load_style_lov(self, wb):
        """Style LOV: Load from 'Clarks MD Mapping' sheet, Col H (program) -> Col I (Style LOV)."""
        sheet_name = next(
            (s for s in wb.sheetnames if "CLARKS" in s.upper() and "MD" in s.upper() and "MAPPING" in s.upper()),
            next((s for s in wb.sheetnames if "MD MAPPING" in s.upper()), None),
        )
        ...
        lov_map: dict[str, str] = {}  # program value (upper) -> Style LOV ID
        for row in rows[hdr_idx + 1:]:
            if not row or len(row) < 9:  # Need at least columns H and I (indices 7 and 8)
                continue
            program_val = str(row[7]).strip() if row[7] else ""  # Col H (index 7)
            style_lov = str(row[8]).strip() if row[8] else ""  # Col I (index 8)
            if program_val and style_lov:
                lov_map[program_val.upper()] = style_lov
        
        self.lovs["StyleLOV"] = lov_map
        self.lovs["AT_Style"] = lov_map
        log.info("[MDD] Style LOV loaded from '%s' — %d entries", sheet_name, len(lov_map))
...
class RNALoader:
    ...
    def _load(self):
        ...
        log.info("[RNA] Full path: %s", self.path.absolute())
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)
        log.info("[RNA] All sheets in brand mapping file: %s", wb.sheetnames)
...
class ClarksOrderFormACCsLoader:
    """
    Loads Clarks Order Form Accs data from the 'Order Form' sheet.
    ...
    Key columns (confirmed from file inspection):
        ...
        SourceCountry   (col T / index 19) — country of origin
    ...
    def _load(self):
        log.info("[Clarks-OrderFormACCs] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)
        log.info("[Clarks-OrderFormACCs] Available sheets: %s", wb.sheetnames)

        # Try "Order Form" first, then first sheet
        target = "Order Form" if "Order Form" in wb.sheetnames else wb.sheetnames[0]
        log.info("[Clarks-OrderFormACCs] Using sheet: '%s'", target)
        ...
        if not rows:
            log.warning("[Clarks-OrderFormACCs] Sheet '%s' is empty", target)
            return
        ...
        log.info("[Clarks-OrderFormACCs] Header at row %d (0-indexed)", hdr_idx)
        ...
        # ── Filter: keep rows with a valid ProductNumber ──────────
        prod_col = self._find_col(header, ["ProductNumber", "productnumber", "Product Number"])
        if prod_col:
            df = df[
                df[prod_col].notna()
        ...
        log.info(
            "[Clarks-OrderFormACCs] Loaded %d data rows | headers: %s",
...
def group_rows(
    ...
    """
    # SlotNumber = principal style code (groups colour variants into one generic)
    col_slot_number  = loader.find_col("SlotNumber", "Slot Number")
    # ProductNumber = EAN barcode (one per colour row)
    col_ean          = loader.find_col("ProductNumber", "Product Number")
    col_style_name   = loader.find_col("StyleName", "Style Name", "Style_Name")
    col_colour       = loader.find_col("MediumColour", "Medium Colour", "Colour")
    col_colour_id    = loader.find_col("ColourID", "Colour ID", "ColourId")
    col_gender       = loader.find_col("GenderName", "Gender Name", "Gender_Name")
    col_country      = loader.find_col("SourceCountry", "Source Country")
    ...
    log.info("[Grouper] Starting to process %d rows", len(raw_rows))
    if raw_rows:
        log.info("[Grouper] Sample row (first): keys=%s", list(raw_rows[0].keys())[:10])
    ...
    for row in raw_rows:
        ...
        if generic_key not in result:
            ...
            # Gender SAP mapping (two-step: dictionary → MDD LOV)
            ...
            # GenderName holds values like "Mens Accessories" — for AT_Gender
            # only the first word ("Mens") is used to look up the SAP mapping.
            gender_first_word = gender_raw.split()[0] if gender_raw else gender_raw
            gender_key  = gender_first_word.strip().upper()
            ...
            result[generic_key] = {
                ...
                "AT_PrincipalSize":                 "No Size",   # Acc: Default (No Size)
                "AT_PrincipalMerchandiseHierarchyL1": bu_desc,
                "AT_PrincipalMerchandiseHierarchyL2": "",  # Manual input
                "AT_PrincipalMerchandiseHierarchyL3": gender_raw,
                "AT_PrincipalMerchandiseHierarchyL4": program,
                "AT_Collection1":                   collection1_lov_id,
                "AT_Style":                         style_lov_id,
    ...
    log.info(
        "[Grouper] Skipped rows: no_ean=%d, total_processed=%d",
        skipped_no_ean, total_rows_processed,
    )
    if not result:
        log.warning("[Grouper] *** NO GENERICS PRODUCED! ***")
        log.warning("[Grouper] Check if col_ean='%s' has valid data", col_ean)
        log.warning("[Grouper] Available columns: %s", loader.headers[:20])
        if raw_rows:
            log.warning("[Grouper] Sample of first row data: %s",
                       {k: v for k, v in list(raw_rows[0].items())[:10]})
    return result
...
def build_product_xml(
    ...
    g_el.set("ParentID",   "PPH_E-TempSubCat")  # Accessories → Equipment hierarchy
    ...
    # ── Country Size (default NS) ─────────────────────────────────
    _val_lov(gv, "AT_CountrySize", "NS")

    # ── Sports Category EN (default 16) ───────────────────────────
    _val_lov(gv, "AT_SportsCategoryEN", "16")

    # ── Images Source (default DI) ────────────────────────────────
...
def run(args, auditor=None):
    """
    Main entry point for Clarks Order Form Accs ETL.
    ...
    log.info(
        "[Clarks-OrderFormACCs] Starting: brand=%s  code=%s  season=%s",
    ...
    if mdd_file:
        ...
        log.warning("[Clarks-OrderFormACCs] No MDD file found in %s — LOV lookups disabled", MDD_DIR)

    # ── Validate brand mapping file + load RNA ───────────────────
    log.info("[Clarks-OrderFormACCs] Looking for brand mapping file in: %s", ATTR_DIR)
    log.info("[Clarks-OrderFormACCs] Files in ATTR_DIR: %s", list(ATTR_DIR.glob("*")))
    mapping_file = next(ATTR_DIR.glob("*.xlsx"), None) or next(ATTR_DIR.glob("*.xlsm"), None)
    rna = None
    if mapping_file:
        log.info("[Clarks-OrderFormACCs] Brand mapping file found: %s", mapping_file)
        log.info("[Clarks-OrderFormACCs] Brand mapping file absolute path: %s", mapping_file.absolute())
        rna = RNALoader(mapping_file)
    else:
        log.warning("[Clarks-OrderFormACCs] Brand mapping file not found in %s", ATTR_DIR)

    # ── Load input Order Form Accs file ──────────────────────────
    input_file = next(INPUT_DIR.glob("*.xls*"), None)
    if not input_file:
        raise FileNotFoundError(f"No Order Form Accs file found in {INPUT_DIR}")
    ...
    if len(parts) >= 3:
        ...
        if brand_token:
            ...
            log.info(
                "[Clarks-OrderFormACCs] Brand extracted from filename: '%s' → '%s'",
                brand_token, brand_from_file,
            )
    if len(parts) >= 1 and parts[0].strip():
        raw_comp = parts[0].strip().lstrip("0") or "0"  # '0888' → '888'
        filename_comp_code = raw_comp
        log.info("[Clarks-OrderFormACCs] Comp code from filename: '%s' → '%s'", parts[0].strip(), filename_comp_code)
    if len(parts) >= 2 and parts[1].strip():
        filename_sbu = parts[1].strip().upper()   # e.g. 'FF'
        log.info("[Clarks-OrderFormACCs] SBU from filename: '%s'", filename_sbu)
    if len(parts) >= 2:
        ...
        if re.match(r'^[A-Z]{2}$', candidate):   # 2-letter ISO code
            ...
            log.info("[Clarks-OrderFormACCs] Country ID from filename: '%s'", filename_country_id)
    ...
    if mdd:
        ...
        if brand_lov:
            ...
            log.info(
                "[Clarks-OrderFormACCs] Brand LOV lookup: key='%s' → LOV ID='%s' %s",
                brand_key, brand_lov_id,
                "(FOUND)" if looked_up else f"(NOT FOUND — fallback to '{brand_code}')",
            )
            if not looked_up:
                log.warning(
                    "[Clarks-OrderFormACCs] '%s' not in Brand LOV. "
                    "Available keys: %s",
                    brand_key, list(brand_lov.keys()),
                )
        else:
            log.warning("[Clarks-OrderFormACCs] Brand LOV not found in MDD — using fallback brand_code='%s'", brand_code)
    else:
        log.warning("[Clarks-OrderFormACCs] No MDD loaded — brand_lov_id defaulting to '%s'", brand_code)
    ...
    if filename_country_id and mdd:
        ...
        log.info(
            "[Clarks-OrderFormACCs] Country: ID='%s' → name='%s' (for RNA lookup)",
            filename_country_id, rna_country,
        )
        if not rna_country:
            log.warning("[Clarks-OrderFormACCs] Country ID '%s' not found in Country LOV", filename_country_id)
    if not rna_country:
    ...
    loader = ClarksOrderFormACCsLoader(input_file)

    if loader.df.empty:
        log.warning("[Clarks-OrderFormACCs] No data rows found — nothing to process")
        return
    ...
    log.info("[Clarks-OrderFormACCs] Writing XML → %s", xml_filename)
    with open(xml_path, "w", encoding="utf-8", buffering=1 << 20) as f:
        ...
        log.info(
            "[Clarks-OrderFormACCs] Processing %d generics",
            len(generics),
        )
    ...
    log.info(
        "[Clarks-OrderFormACCs] XML written: %s  (%d KB)",
        xml_path.name, file_kb,
    )
    print(
        f"=== CLARKS ORDER FORM ACCS SUMMARY ===\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Clarks Order Form Accs → Stibo XML")
