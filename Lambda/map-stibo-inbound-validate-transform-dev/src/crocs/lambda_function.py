"""
lambda_function.py — Lambda Handler for Crocs
================================================
Function name : map-stibo-inbound-validate-transform-crocs-dev

Trigger:
    EventBridge rule (S3 Object Created) scoped to:
        s3://map-stibo-inbound-raw-dev/raw/metadata/crocs/*

Crocs-specific flow:
    • ONE primary input file from the brand : linelist (SEA line list)
...
File-type dispatch:
    The triggered S3 key determines which ETL module is called:
        linelist     → crocs.linelist_dynamic.run()

S3 layout:
    raw/metadata/crocs/{linelist_file}.xlsx           ← Crocs uploads here
...
Output:
    s3://map-stibo-inbound-processed-dev/
        processed/stepxml/crocs/{filename}.xml
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Set LAMBDA_TMP_DIR BEFORE importing crocs modules so that their
# module-level BASE_DIR resolves to /tmp/stibo_workdir_crocs
# (isolated from other brands).
# ─────────────────────────────────────────────────────────────────────────────
TMP_WORKDIR = "/tmp/stibo_workdir_crocs"
os.environ["LAMBDA_TMP_DIR"] = TMP_WORKDIR

# ── ETL modules — Crocs linelist and shipment_confirmation ─────────────────
import crocs.linelist  as linelist_etl     # noqa: E402  # Dynamic mapping from brand file
import crocs.shipment_confirmation_main  as shipment_confirmation_etl  # noqa: E402  # Shipment Confirmation EAN
import crocs.order_form_jbz_ean_main as order_form_jbz_ean_etl  # noqa: E402  # Order Form JBZ EAN
import crocs.order_form_fw_main as order_form_fw_etl  # noqa: E402  # Order Form FW EAN
import crocs.order_form_accs_bag_main as order_form_accs_bag_etl  # noqa: E402  # Order Form ACCs BAG EAN
# ─────────────────────────────────────────────────────────────────────────────
# Logging
# ─────────────────────────────────────────────────────────────────────────────
logging.basicConfig(
...
log = logging.getLogger("lambda_crocs")
...
FILE_TYPE_KEYWORDS: dict[str, list[str]] = {
    # Crocs Order Form FW (EAN processing)
    # Example: "0888-SP-CROCS-Order Form FW (MAA Sport) Inline-Multi-SP2029-KH-1.xlsx"
    "order_form_fw": [
        "order form fw",
    ],
    # Crocs Order Form JBZ (EAN processing)
    # Example: "0888-SP-CROCS-Order Form JBZ (MAA Sport) Inline-Multi-SP2029-KH-1.xlsx"
    "order_form_jbz": [
        "order form jbz",
    ],
    # Crocs Order Form ACCs BAG (EAN processing)
    # Example: "0888-SP-CROCS-Order Form ACCs BAG (MAA Sport) Inline-Multi-SP2029-KH-1.xlsx"
    "order_form_accs_bag": [
        "order form accs bag",
    ],
    # Crocs primary input (SEA line list / FTW / Charms)
    # Example: "H1 27 SEA Line List - SEA (5.15).xlsx"
    "linelist": [
        "line list", "linelist",
        "sea line list",
        "ftw", "charms",
    ...
    # Crocs shipment confirmation
    # Example: "Shipment Confirmation - Early Ready (Jan orders) (12122025).xlsx"
...
REQUIRED_TYPES_BY_TRIGGER: dict[str, set[str]] = {
    ...
    "shipment_confirmation": {"shipment_confirmation", "mdd", "attributes"},
    "order_form_jbz": {"order_form_jbz", "mdd", "attributes"},
    "order_form_fw": {"order_form_fw", "mdd", "attributes"},
    "order_form_accs_bag": {"order_form_accs_bag", "mdd", "attributes"},
}

REQUIRED_TYPES_DEFAULT: set[str] = {"linelist", "mdd", "attributes"}

# Global types fetched from raw/metadata/ root (not inside crocs/)
GLOBAL_TYPES: set[str] = {"mdd", "attributes"}

# ── ETL dispatcher ────────────────────────────────────────────────────────────
ETL_DISPATCHER: dict[str, object] = {
    "linelist": linelist_etl,
    "shipment_confirmation": shipment_confirmation_etl,
    "order_form_jbz": order_form_jbz_ean_etl,
    "order_form_fw": order_form_fw_etl,
    "order_form_accs_bag": order_form_accs_bag_etl,
}
...
def _detect_file_type(filename: str) -> str | None:
    ...
    # Check against all file type keywords
...
def _detect_triggered_file_type(key: str) -> str | None:
    ...
    Returns a key from FILE_TYPE_KEYWORDS or None for global fan-out.
    """
...
def _extract_principal(key: str) -> str | None:
    ...
    Returns the principal (e.g. "crocs") or None.
    """
...
def _list_principal_files(
    ...
    Scan S3 for all recognised files:
      ...
      2. Global shared files   ← raw/metadata/           (mdd + attributes)
      3. Bucket root           ← s3://bucket/            (attributes — always checked,
                                                           overrides Step 2)

    Brand-specific takes priority if both exist for the same type.
    Bucket root always wins for attributes (file stored at s3://bucket/ root level).
    """
    ...
    # ── Step 2: global folder — now skipped for Crocs ────────────
    # Crocs fetches both MDD and attributes from bucket root only
    
    # ── Step 3: bucket root — always checked for MDD + attributes ─
    # Both MDD and Brand Mapping files are stored at bucket root
    try:
        ...
        for obj in resp.get("Contents", []):
            ...
            # Check for attributes (brand mapping)
            if ftype == "attributes" and "attributes" not in exclude_types:
                ...
                log.info(
                    "  Classified  attributes    ← %s  [bucket root]", filename,
                )
            ...
            # Check for MDD
                ...
                log.info(
                    "  Classified  mdd           ← %s  [bucket root]", filename,
                )
    except Exception as e:
...
def _parse_metadata_from_filename(
    ...
    Expected format:
        {CompanyCode}-{SBU}-{Brand}-{FileType}-{Multi/Mono}-{Season}-{Country}-{Seq}.xlsx
    e.g. 0888-FL-Crocs-Line List SEA-Multi-FW26-MY-1.xlsx
    """
    ...
    for folder in candidate_dirs:
        ...
        for xlsx_file in xls_files:
            ...
            # Supports both:
            #   "CROCS"         → brand="Crocs",  brand_code="CCR"  (default)
            #   "CROCS CCR"     → brand="Crocs",  brand_code="CCR"  (embedded)
            #   "CROCS JBZ"     → brand="Crocs",  brand_code="JBZ"  (embedded)
            brand      = "Crocs"   # default
            brand_code = "CCR"     # default
            ...
            if brand_token_match:
                # e.g. "CROCS CCR" → brand="Crocs", brand_code="CCR" (from filename)
                ...
                # No embedded code — use defaults: brand="Crocs", brand_code="CCR"
...
def _prepare_tmp_dirs() -> dict[str, Path]:
    """Create clean /tmp/stibo_workdir_crocs/input/{type}/ dirs."""
    ...
    dirs: dict[str, Path] = {
        ...
        "shipment_confirmation":   base / "input" / "shipment_confirmation",
        "order_form_jbz":          base / "input" / "order_form_jbz",
        "order_form_fw":           base / "input" / "order_form_fw",
        "order_form_accs_bag":      base / "input" / "order_form_accs_bag",
        "mdd":                     base / "input" / "mdd",
...
def lambda_handler(event, context, auditor: AuditLogger = None):
    ...
    if _direct_invocation:
        ...
        auditor.set_router_context(
            ...
            original_key     = "",
            detected_brand   = "crocs",
            routing_decision = "crocs lambda_handler invoked directly",
        )
    ...
    if etl_module is None:
        # Try to resolve from available files: linelist first, then shipment_confirmation
        for file_type in ["linelist", "shipment_confirmation"]:
            if file_type in found:
                etl_module          = ETL_DISPATCHER.get(file_type)
                triggered_file_type = file_type
                log.info(
                    "Global fan-out: resolved ETL module from present files → '%s'", file_type,
                )
                break
    ...
    # Safety guard — brand_code must be CCR (or an embedded 3-letter code from filename)
    if meta["brand_code"] != "CCR":
        log.warning(
            "Filename resolved brand_code='%s' — expected 'CCR' (default) or an "
            "embedded 3-letter code in the filename (e.g. 'CROCS JBZ'). "
            "Proceeding but verify filename format.",
            meta["brand_code"],
        )
    ...
    log.info(
        "Running Crocs ETL → %s (triggered_file_type=%s) ...",
    ...
    linelist_info = found.get(triggered_file_type, found.get("linelist", {}))
    return {
        "statusCode":          200,
        "principal":           principal,
        "triggered_file_type": triggered_file_type,
        "source_file":         linelist_info.get("filename"),
        "source_s3_key":       linelist_info.get("key"),
        "uploaded":            uploaded,
        "count":               len(uploaded),
        "status":              "ok",
    }