...
def _sample_prefix(sku: str) -> str:
    ...
    """
    raw = (sku or '').strip()
    # Strip the trailing "_XXX" colour suffix (e.g. LAS65129_AS4 → LAS65129)
    if '_' in raw:
...
def _build_generic_description(
    ...
    Example output: "NEW SPL SP27 Youth Active No Show Socks (AD/U) GY"
...
def _add_generic_values(vals_el, art, brand_name, comp_code, sbu, mdd=None, bm=None):
    ...
    # Accessories (E): CLH_EZYZY → LOV ID EZYZYNEWA
    _val(vals_el, "AT_MerchandiseCategory", "SAMPLE", id_val="EZYZYNEWA")

    # ── Issue #5 (24-09-2026): BY Sub Category — Accessories Sample hierarchy
    # BY Hierarchy: MA_NEW_E_ZY_ZY_ZY → "Accessories Sample"
    _val(vals_el, "AT_BYSubCategory", "Accessories Sample", id_val="MA_NEW_E_ZY_ZY_ZY")
