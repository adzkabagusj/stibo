"""
╔══════════════════════════════════════════════════════════════════╗
║   STIBO INBOUND XML GENERATOR — Asics Footwear v1.0             ║
║   Footwear Order Form → Stibo STEP XML                          ║
...
class MDDLoader:
    """
    Loads LOV tables from the MDD Excel needed for Asics Footwear.
...
class BrandMappingLoader:
    ...
    def _load(self):
        ...
        if not rows:
            log.warning("[BrandMapping] Sheet '%s' is empty", self.sheet_name)
            return
...
class AsicsFootwearLoader:
    """
    Loads Asics Footwear data from Excel.
    ...
    def _load(self):
        log.info("[AsicsFootwear] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)
        log.info("[AsicsFootwear] Available sheets: %s", wb.sheetnames)

        # Prefer "AW26 OTB Detail" sheet; fall back to first sheet
        PREFERRED_SHEETS = ["AW26 OTB Detail"]
        target = next(
            (s for s in wb.sheetnames if s.strip() in PREFERRED_SHEETS),
            wb.sheetnames[0],
        )
        log.info("[AsicsFootwear] Using sheet: '%s'", target)
        ...
        if not rows:
            log.warning("[AsicsFootwear] Sheet '%s' is empty", target)
            return

        # Locate header row by looking for "Item Color Code" in any cell
        hdr_idx = 0
        for i in range(min(20, len(rows))):
            if rows[i] and any(
                str(v).strip() == "Item Color Code"
                for v in rows[i] if v is not None
            ):
                hdr_idx = i
                log.info("[AsicsFootwear] Header detected at row %d (found 'Item Color Code')", hdr_idx)
                break
        else:
            log.warning("[AsicsFootwear] 'Item Color Code' not found in first 20 rows — defaulting to row 0")
        ...
        log.info(
            "[AsicsFootwear] Loaded %d data rows | %d columns",
...
def group_rows_dynamic(
    ...
    col_principal_style = loader.find_col(
        "Item Color Code",  # Asics-specific for AT_PrincipalStyleCode
    )
    ...
    col_gender = loader.find_col(
        "Gender",  # Asics-specific for AT_PrincipalGenderDescription
    )
    col_aid_srp = loader.find_col(
        "AID SRP",  # Asics-specific for AT_OriginalPrice
    )
    col_fw_pillar = loader.find_col(
        "FW Pillar",  # Asics-specific for AT_PrincipalMerchandiseHierarchyL1
    )
    col_silo1 = loader.find_col(
        "Silo 1",  # Asics-specific for AT_PrincipalMerchandiseHierarchyL2
    )
    col_series = loader.find_col(
        "Series",  # Asics-specific for AT_PrincipalMerchandiseHierarchyL3
    )
    ...
    col_incoming_month = loader.find_col(
        "AID RID",  # Asics-specific for AT_IncomingMonth
    )

    log.info(
        "[Grouper] Key columns → style=%s, principal_style=%s, working_name=%s, colorway=%s, gender=%s, aid_srp=%s, fw_pillar=%s, silo1=%s, series=%s, width=%s, color=%s, size=%s, ean=%s",
        col_style, col_principal_style, col_working_name, col_colorway, col_gender, col_aid_srp, col_fw_pillar, col_silo1, col_series, col_width, col_color, col_size, col_ean,
    )
    ...
    for row in raw_rows:
        ...
        if generic_key not in result:
            ...
            fw_pillar = _s(row.get(col_fw_pillar, "")) if col_fw_pillar else ""
            silo1 = _s(row.get(col_silo1, "")) if col_silo1 else ""
            series = _s(row.get(col_series, "")) if col_series else ""
...
def build_product_xml_dynamic(
    ...
    # ── AT_PrincipalSize: from Sizes column ──
    ...
    # ── AT_PrincipalMerchandiseHierarchyL1: from FW Pillar column ──
    if generic.get("fw_pillar"):
        _val_text(gv, "AT_PrincipalMerchandiseHierarchyL1", generic["fw_pillar"])

    # ── AT_PrincipalMerchandiseHierarchyL2: from Silo 1 column ──
    if generic.get("silo1"):
        _val_text(gv, "AT_PrincipalMerchandiseHierarchyL2", generic["silo1"])

    # ── AT_PrincipalMerchandiseHierarchyL3: from Series column ──
    if generic.get("series"):
        _val_text(gv, "AT_PrincipalMerchandiseHierarchyL3", generic["series"])

    # ── AT_Width: from Width column with transformation and LOV lookup ──
    if generic.get("width"):
        width_raw = generic["width"]
        # Transform Excel values to Width LOV display values
        width_mapping = {
            "STANDARD": "REGULAR",
            "WIDE": "WIDE",
        }
        width_display = width_mapping.get(width_raw.upper(), width_raw.upper())
        
        # Look up LOV ID from Width LOV (Col B → Col A mapping)
        width_lov = mdd.lovs.get("AT_Width", {}) if mdd else {}
        width_lov_id = width_lov.get(width_display, "")
        
        if width_lov_id:
            _val_lov(gv, "AT_Width", width_lov_id)
        else:
            # Fallback: use transformed display value as text
            _val_text(gv, "AT_Width", width_display)
    ...
    # ── Silhouette (default 20) ───────────────────────────────────
    _val_lov(gv, "AT_Silhouette", "20")

    # ── Material Upper (default SYN) ──────────────────────────────
    _val_lov(gv, "AT_MaterialUpper", "SYN")

    # ── Fastening (default LU) ────────────────────────────────────
    _val_lov(gv, "AT_Fastening", "LU")

    # ── AT_Franchise: from Series column ─────────────────────────
    if generic.get("series"):
        _val_text(gv, "AT_Franchise", generic["series"])

    # ── EComAgesCategory (default 18+Y) ──────────────────────────
    _val_lov(gv, "AT_EComAgesCategory", "18+Y")

    # ── AT_SPUGroupingName: from Series column ────────────────────
    if generic.get("series"):
        _val_text(gv, "AT_SPUGroupingName", generic["series"])
...
def run(args, auditor=None):
    """
    Main entry point for Asics Footwear ETL.
    ...
    log.info(
        "[AsicsFootwear] Starting: brand=%s  code=%s  season=%s",
    ...
    if mdd_file:
        ...
        log.warning("[AsicsFootwear] No MDD file found in %s — LOV lookups disabled", MDD_DIR)
    ...
    if not brand_mapping.get_mappings():
        log.warning("[AsicsFootwear] No attribute mappings loaded from 'Asics(Inline)' tab — continuing with hardcoded attributes only")
    ...
    log.info("[AsicsFootwear] Asics MD Mappings loaded: %d FTW entries", len(asics_md_mappings.mappings))

    # Load RNA for brand attributes
    rna = RNALoader(mapping_file)
    log.info("[AsicsFootwear] RNA loaded: %d entries", len(rna.lookup))

    # Load input file
    input_file = next(INPUT_DIR.glob("*.xls*"), None)
    if not input_file:
        raise FileNotFoundError(f"No Asics footwear file found in {INPUT_DIR}")

    # ── Extract brand name from filename ──────────────────────────
    # Pattern: 0999-SP-ASICS-Footwear-SP2027-ID-1.xlsx
    ...
    if len(parts) >= 3:
        ...
        if brand_token:
            ...
            log.info(
                "[AsicsFootwear] Brand extracted from filename: '%s' → '%s'",
                brand_token, brand_from_file,
            )
    if len(parts) >= 1 and parts[0].strip():
        raw_comp = parts[0].strip().lstrip("0") or "0"  # '0999' → '999'
        filename_comp_code = raw_comp
        log.info("[AsicsFootwear] Comp code from filename: '%s' → '%s'", parts[0].strip(), filename_comp_code)
    if len(parts) >= 2 and parts[1].strip():
        filename_sbu = parts[1].strip().upper()   # e.g. 'SP'
        log.info("[AsicsFootwear] SBU from filename: '%s'", filename_sbu)
    if len(parts) >= 2:
        ...
        if re.match(r'^[A-Z]{2}$', candidate):   # 2-letter ISO code
            ...
            log.info("[AsicsFootwear] Country ID from filename: '%s'", filename_country_id)
    ...
    if mdd:
        ...
        if brand_lov:
            ...
            log.info(
                "[AsicsFootwear] Brand LOV lookup: key='%s' → LOV ID='%s' %s",
                brand_key, brand_lov_id,
                "(FOUND)" if looked_up else f"(NOT FOUND — fallback to '{brand_code}')",
            )
            if not looked_up:
                log.warning(
                    "[AsicsFootwear] '%s' not in Brand LOV. "
                    "Available keys: %s",
                    brand_key, list(brand_lov.keys()),
                )
        else:
            log.warning("[AsicsFootwear] Brand LOV not found in MDD — using fallback brand_code='%s'", brand_code)
    else:
        log.warning("[AsicsFootwear] No MDD loaded — brand_lov_id defaulting to '%s'", brand_code)
    ...
    if filename_country_id and mdd:
        ...
        log.info(
            "[AsicsFootwear] Country: ID='%s' → name='%s' (for RNA lookup)",
            filename_country_id, rna_country,
        )
        if not rna_country:
            log.warning("[AsicsFootwear] Country ID '%s' not found in Country LOV", filename_country_id)
    if not rna_country:
    ...
    loader = AsicsFootwearLoader(input_file)

    if loader.df.empty:
        log.warning("[AsicsFootwear] No data rows found — nothing to process")
        return
    ...
    if not generics:
        log.warning("[AsicsFootwear] No valid generics produced")
        return
    ...
    log.info("[AsicsFootwear] Writing XML → %s", xml_filename)
    ...
    log.info(
        "[AsicsFootwear] XML written: %s  (%d KB)",
        xml_path.name, file_kb,
    )
    print(
        f"=== ASICS FOOTWEAR SUMMARY ===\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Asics Footwear → Stibo XML")
