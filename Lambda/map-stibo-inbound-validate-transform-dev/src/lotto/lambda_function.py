"""
lambda_function.py — Lambda Handler for Lotto (LOT)
====================================================
Function name : map-stibo-inbound-validate-transform-lotto-dev

Trigger:
    EventBridge rule (S3 Object Created) scoped to:
        s3://map-stibo-inbound-raw-dev/raw/metadata/lotto/*
...
Output:
    s3://map-stibo-inbound-processed-dev/
        processed/stepxml/lotto/{filename}.xml
"""
...
# ─────────────────────────────────────────────────────────────────────────────
TMP_WORKDIR = "/tmp/stibo_workdir_lotto"
...
log = logging.getLogger("lambda_lotto")
...
def _lotto_default_metadata(dirs: dict) -> dict:
    """
...
def lambda_handler(event, context, auditor: AuditLogger = None):
    ...
    if _direct_invocation:
        ...
        auditor.set_router_context(
            ...
            detected_brand="lotto", routing_decision="lotto lambda_handler invoked directly",
        )
    ...
    meta = _lotto_default_metadata(dirs)
    log.info("[Lotto] Using metadata: %s", meta)
    ...
    log.info("Running Lotto ETL → %s ...", etl_module.__name__)
