...
# ─────────────────────────────────────────────────────────────────────────────
TMP_WORKDIR = "/tmp/stibo_workdir_anta"
os.environ["LAMBDA_TMP_DIR"] = TMP_WORKDIR

# ── ETL modules ──────────────────────────────────────────────────────────────
import anta.linelist_main as linelist_etl  # noqa: E402
import anta.ean_main as ean_etl            # noqa: E402
...
log = logging.getLogger("lambda_anta")
...
def lambda_handler(event, context, auditor: AuditLogger = None):
    ...
    if _direct_invocation:
        ...
        auditor.set_router_context(
            ...
            detected_brand="anta", routing_decision="anta lambda_handler invoked directly",
        )
