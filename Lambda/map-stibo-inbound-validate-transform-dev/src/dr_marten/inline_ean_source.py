"""
╔══════════════════════════════════════════════════════════════════╗
║   STIBO INBOUND XML GENERATOR — Dr. Martens Inline EAN  v1.0     ║
║   Dr. Martens EAN Source report → Stibo STEP XML                 ║
║   (Structure mirrors birkenstock/oor.py)                         ║
╚══════════════════════════════════════════════════════════════════╝

Input file: Dr. Martens EAN Source (Excel)  — "5. DR.MARTENS_EAN Source.xlsx"
    Sheet   : Sheet1
    Style   : Item number      (col J)  -> AT_PrincipalStyleCode
    Size    : Size             (col M)  -> Principal Size attribute
    Barcode : Bar code         (col AC) -> AT_Barcode

KEY_InboundArticle formula:
    BrandCode + Item number   (e.g. DRM31813001)
...
XML shape mirrors Birkenstock EAN: Generic <Values/> empty, Variant
<Values/> empty, barcode goes into DC_Barcode DataContainer.
"""
...
# ── TESTING LIMITS (commented out — processes ALL rows/products/variants) ──
# Uncomment the three lines below + the TEST MODE block in run() to re-enable.
# TEST_MODE = True
# TEST_MAX_PRODUCTS = 2
...
class DRMEANLoader:
    """Loads Dr. Martens EAN Source data from the 'Sheet1' sheet."""
    ...
    def _load(self):
        log.info("[DrMartens-EAN] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)

        target = None
        for sn in wb.sheetnames:
            if sn.strip().upper() == self.SHEET_NAME.upper():
                target = sn
                break
        if not target:
            log.warning("[DrMartens-EAN] Sheet 'Sheet1' not found — using first sheet")
            target = wb.sheetnames[0]

        log.info("[DrMartens-EAN] Using sheet: '%s'", target)
        ws = wb[target]
        rows = list(ws.iter_rows(values_only=True))
        wb.close()

        if not rows:
            log.warning("[DrMartens-EAN] Sheet '%s' is empty", target)
            return

        # Header row = first row with an "Item number" column
        hdr_idx = 0
        for i in range(min(15, len(rows))):
            if rows[i]:
                rv = {str(v).strip().lower() for v in rows[i] if v is not None}
                if "item number" in rv:
                    hdr_idx = i
                    break

        log.info("[DrMartens-EAN] Header at row %d (0-indexed)", hdr_idx)
        ...
        style_col = self._find_column(header, ["Item number"])
        if style_col:
        ...
        log.info("[DrMartens-EAN] %d rows loaded", len(self.df))
        log.info("[DrMartens-EAN] Headers: %s", self.headers[:30])
...
def _clean_style(style_val) -> str:
    ...
    if s in ("None", "nan", ""):
        return ""
    # Item number may come through as a float (e.g. 26957001.0)
    try:
        f = float(s)
        if f.is_integer():
            return str(int(f))
...
def group_ean_rows(raw_rows: list[dict], brand_code: str,
                   loader: DRMEANLoader, mdd=None) -> dict[str, dict]:
    """
    KEY_InboundArticle = BrandCode + Item number  (e.g. DRM31813001)
    KEY_InboundVariant = KEY_InboundArticle + Size LOV ID

    Each unique Item number = One Generic.
    Each row = One Variant (Size = size, Bar code = barcode).
    """
    result: dict[str, dict] = {}

    style_col = loader.find_column("Item number")
    size_col  = loader.find_column("Size")
    ean_col   = loader.find_column("Bar code", "Barcode", "Bar Code", "EAN", "EAN-Code", "EAN Code")

    if not style_col:
        log.error("[DrMartens-EAN] Cannot find 'Item number' column")
        return result

    log.info("[DrMartens-EAN] All available headers: %s", loader.headers)
    log.info("[DrMartens-EAN] Using columns: Style='%s', Size='%s', Barcode='%s'",
             style_col, size_col, ean_col)

    if not size_col:
        log.warning("[DrMartens-EAN] Size column NOT FOUND - sizes will default to 'MSC'!")
    if not ean_col:
        log.warning("[DrMartens-EAN] Bar code column NOT FOUND - barcodes will be empty!")

    size_lov = mdd.lovs.get("Size Code") if mdd else None
    if size_lov:
        log.info("[DrMartens-EAN] Size Code LOV loaded with %d entries", len(size_lov))
    else:
        log.warning("[DrMartens-EAN] Size Code LOV not found in MDD - using fallback formulas")
    ...
    for row in raw_rows:
        ...
        if generic_code not in result:
            result[generic_code] = {
                ...
                "division": "F",  # Footwear default for Dr. Martens
                "variants": {},
            }
...
def build_product_xml(generic: dict) -> str:
    ...
    for var_key in generic["variants"].keys():
...
def run(args, auditor=None):
    """Main entry point for Dr. Martens Inline EAN processing."""
    brand      = getattr(args, "brand", BRAND_NAME)
    brand_code = getattr(args, "brand_code", BRAND_CODE)

    log.info("[DrMartens-EAN] Starting: brand=%s code=%s", brand, brand_code)
    ...
    if mdd_file:
        ...
        log.warning("[DrMartens-EAN] No MDD file found in %s", MDD_DIR)
    ...
    loader = DRMEANLoader(ean_file)
    if loader.df.empty:
        log.warning("[DrMartens-EAN] No data rows found")
        return
    ...
    if not generics:
        log.warning("[DrMartens-EAN] No valid generics produced")
        return
    # ── Test mode: cap to N products, M variants per product ─────────
    # (Commented out — processing ALL products/variants. Re-enable with the
    #  TEST_* constants above to limit output for testing.)
    # if TEST_MODE:
    ...
    log.info("[DrMartens-EAN] XML written: %s  (%d KB)", xml_path.name, file_kb)
    print(f"=== DR. MARTENS INLINE EAN SUMMARY ===\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Dr. Martens Inline EAN -> Stibo XML")
