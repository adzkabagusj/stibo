...
def map_sku(row: pd.Series, brand_code: str = "NEW") -> dict:
    ...
    # Build style_desc for generic description (SPL + season will be injected in _add_generic_values)
    # Pass a placeholder here; the actual season-prefixed value is built at XML-write time.
    # We store raw display_name and let _add_generic_values compute generic_desc with season.
...
def _add_generic_values(vals_el, art, brand_name, comp_code, sbu, mdd=None, bm=None):
    ...
    # Footwear (F): CLH_FZYZY → LOV ID FZYZYNEWA
    _val(vals_el, "AT_MerchandiseCategory", "SAMPLE", id_val="FZYZYNEWA")

    # ── Issue #5 (24-09-2026): BY Sub Category — Footwear Sample hierarchy
    # BY Hierarchy: MA_NEW_F_ZY_ZY_ZY → "Footwear Sample"
    _val(vals_el, "AT_BYSubCategory", "Footwear Sample", id_val="MA_NEW_F_ZY_ZY_ZY")
