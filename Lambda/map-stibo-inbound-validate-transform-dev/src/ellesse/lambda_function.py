...
# ─────────────────────────────────────────────────────────────────────────────
TMP_WORKDIR = "/tmp/stibo_workdir_ellesse"
os.environ["LAMBDA_TMP_DIR"] = TMP_WORKDIR

# ── ETL modules ──────────────────────────────────────────────────────────────
import ellesse.recap_main           as recap_etl            # noqa: E402
...
log = logging.getLogger("lambda_ellesse")
...
def _ellesse_default_metadata(dirs: dict) -> dict:
    """
...
def lambda_handler(event, context, auditor: AuditLogger = None):
    ...
    if _direct_invocation:
        ...
        auditor.set_router_context(
            ...
            detected_brand="ellesse", routing_decision="ellesse lambda_handler invoked directly",
        )
    ...
    meta = _ellesse_default_metadata(dirs)
    log.info("[Ellesse] Using metadata: %s", meta)
    ...
    log.info("Running Ellesse ETL → %s ...", etl_module.__name__)
