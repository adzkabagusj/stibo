"""
lambda_function.py — Lambda Handler for Staccato (SC7 / STACCATO)
====================================================================
Function name : map-stibo-inbound-validate-transform-staccato-dev

Trigger:
    EventBridge rule (S3 Object Created) scoped to:
        s3://map-stibo-inbound-raw-dev/raw/metadata/staccato/*

Staccato-specific flow:
    ...
    • Required types: {"linelist"}
    • Keyword detection: "linelist", "linesheet", "staccato", "selection"

File-type dispatch:
    linelist → staccato.linelist_main.run()

S3 layout:
    raw/metadata/staccato/{linelist_file}.xlsx   ← Staccato uploads here
    raw/metadata/mdd_file.xlsx                   ← shared global MDD (optional)

Output:
    s3://map-stibo-inbound-processed-dev/
        processed/stepxml/staccato/{filename}.xml
"""
...
# ─────────────────────────────────────────────────────────────────────────────
TMP_WORKDIR = "/tmp/stibo_workdir_staccato"
os.environ["LAMBDA_TMP_DIR"] = TMP_WORKDIR

# ── ETL modules ──────────────────────────────────────────────────────────────
import staccato.linelist_main as linelist_etl  # noqa: E402
...
log = logging.getLogger("lambda_staccato")
...
FILE_TYPE_KEYWORDS: dict[str, list[str]] = {
    "linelist": [
        ...
        "staccato", "selection consolidation", "article planning",
    ],
...
def _parse_metadata_from_filename(dirs: dict) -> dict:
    ...
    e.g. 0888-SP-STACCATO SC7-Linesheet MAA Sport Inline-Multi-SM2027-VN-1.xlsx
    """
    defaults = {
        "comp_code":     "0888",
        "sbu":           "FF",
    ...
    log.info("[Staccato] Parsing metadata from filename: '%s'", stem)
    parts = re.split(r"\s*-\s*", stem)

    if len(parts) < 6:
        log.warning("[Staccato] Filename has < 6 dash-parts — using defaults: '%s'", stem)
        return defaults
    ...
    log.info(
        "[Staccato] Metadata: comp_code=%s  sbu=%s  season=%s%s  country=%s  article_type=%s",
...
def lambda_handler(event, context, auditor: AuditLogger = None):
    ...
    if _direct_invocation:
        ...
        auditor.set_router_context(
            ...
            detected_brand="staccato", routing_decision="staccato lambda_handler invoked directly",
        )
    ...
    log.info("[Staccato] Using metadata: %s", meta)
    ...
    log.info("[Staccato] Input file resolved: %s", input_file)
    ...
    log.info("Running Staccato ETL → %s ...", etl_module.__name__)
    ...
    try:
        ...
        log.error("[Staccato] ETL run failed: %s", exc)
