...
Column availability audit — Price List columns vs. GTM sheet:
  ...
  ───────────────────────────────
    ...
    Line Plan Business   → AT_SportsCategoryEN (LOV)
...
def _build_sap_style_code(product_number: str) -> str:
    """Unchanged from the price-list ETL: Product Number, alphanumeric, max 9."""
...
def map_sku(row: pd.Series, brand_code: str = "NEW") -> dict:
    ...
    return {
        ...
        "sap_style_code": _build_sap_style_code(product_number),
...
def _add_generic_values(vals_el, art, brand_name, comp_code, sbu, mdd=None):
    ...
    see DROPPED_ATTRIBUTES at the top of this module.
    """
...
def run(args, auditor=None):
    ...
    for ll_path in ll_files:
        ...
        # TEST LIMIT: uncomment the line below to process only the first 5 products
        mapped_skus = mapped_skus[6:10]
