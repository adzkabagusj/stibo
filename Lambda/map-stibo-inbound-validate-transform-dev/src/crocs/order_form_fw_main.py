"""
╔══════════════════════════════════════════════════════════════════╗
║   STIBO INBOUND XML GENERATOR — Crocs Order Form FW v1.0       ║
║   Order Form FW → Stibo STEP XML (Generic + Variants)          ║
╚══════════════════════════════════════════════════════════════════╝

Input file pattern:
    0888-SP-CROCS-Order Form FW (MAA Sport) Inline-Multi-SP2029-KH-1
...
EAN_DIR     = BASE_DIR / "input" / "order_form_fw"  # Lambda downloads order_form_fw here
...
class CrocsOrderFormFWLoader:
    """Loads Crocs Order Form FW source data from specific sheets."""
    ...
    def _load(self):
        log.info("[Crocs-OrderFormFW] Loading: %s", self.path.name)
        wb = openpyxl.load_workbook(self.path, read_only=True, data_only=True)
        
        log.info("[Crocs-OrderFormFW] Available sheets: %s", wb.sheetnames)
        log.info("[Crocs-OrderFormFW] Target sheets: %s", self.TARGET_SHEETS)
        ...
        for sheet_name in self.TARGET_SHEETS:
            ...
            if not target:
                log.warning("[Crocs-OrderFormFW] Sheet '%s' not found, skipping", sheet_name)
                continue
            
            log.info("[Crocs-OrderFormFW] Processing sheet: '%s'", target)
            ws = wb[target]
            rows = list(ws.iter_rows(values_only=True))
            
            if not rows:
                log.warning("[Crocs-OrderFormFW] Sheet '%s' is empty", target)
                continue
            ...
            log.info("[Crocs-OrderFormFW] Sheet '%s': Header at row %d (0-indexed)", target, hdr_idx)
            ...
            log.info("[Crocs-OrderFormFW] Sheet '%s': %d rows loaded", target, len(df))
        ...
        log.info("[Crocs-OrderFormFW] Total: %d rows from %d sheets (%s)", 
                 len(self.df), len(self.sheets_loaded), ", ".join(self.sheets_loaded) if self.sheets_loaded else "none")
        if self.headers:
            log.info("[Crocs-OrderFormFW] Headers: %s", self.headers[:15])
        else:
            log.warning("[Crocs-OrderFormFW] No headers loaded - no matching sheets found")
...
def group_ean_rows(raw_rows: list[dict], brand_code: str, season: str, 
                   country_code: str, loader: CrocsOrderFormFWLoader, 
    ...
    if not style_col:
        log.error("[Crocs-OrderFormFW] Cannot find Style column")
        return result
    
    log.info("[Crocs-OrderFormFW] All available headers: %s", loader.headers)
    log.info("[Crocs-OrderFormFW] Using columns: Style='%s', Size='%s', UPC='%s'", 
             style_col, size_col, upc_col)
    
    if not size_col:
        log.warning("[Crocs-OrderFormFW] Size column NOT FOUND - sizes will default to '000'!")
    
    if not upc_col:
        log.warning("[Crocs-OrderFormFW] UPC column NOT FOUND - barcodes will be empty!")
    
    # Get size LOV from MDD
    size_lov = mdd.lovs.get("Size Code") if mdd else None
    if size_lov:
        log.info("[Crocs-OrderFormFW] Size Code LOV loaded with %d entries", len(size_lov))
    else:
        log.warning("[Crocs-OrderFormFW] Size Code LOV not found in MDD - using fallback formulas")
...
def run(args, auditor=None):
    """Main entry point for Crocs Order Form FW EAN processing."""
    ...
    log.info("[Crocs-OrderFormFW] Starting: brand=%s code=%s season=%s", brand, brand_code, season)
    ...
    if mdd_file:
        ...
        log.warning("[Crocs-OrderFormFW] No MDD file found in %s", MDD_DIR)
    ...
    if not mapping_file:
        for pattern in ["*brand*mapping*.xlsx", "*mapping*template*.xlsx", "*mapping*.xlsx"]:
            ...
            if candidates:
                ...
                break
    if mapping_file:
        log.info("[Crocs-OrderFormFW] Brand mapping file validated: %s", mapping_file.name)
    else:
        log.warning("[Crocs-OrderFormFW] Brand mapping file not found in %s", ATTR_DIR)

    # Load input file
    ean_file = next(EAN_DIR.glob("*.xls*"), None)
    if not ean_file: 
        raise FileNotFoundError(f"No Order Form FW file in {EAN_DIR}")
    
    loader = CrocsOrderFormFWLoader(ean_file)
    
    if loader.df.empty:
        log.warning("[Crocs-OrderFormFW] No data rows found")
        return
    ...
    if not generics:
        log.warning("[Crocs-OrderFormFW] No valid generics produced")
        return

    # Use all generic products (no limit)
    limited_generics = list(generics.values())
    log.info("[Crocs-OrderFormFW] Processing all products: %d generics",
    ...
    log.info("[Crocs-OrderFormFW] XML written: %s  (%d KB)", xml_path.name, file_kb)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Crocs Order Form FW EAN -> Stibo XML")
