...
# ─────────────────────────────────────────────────────────────────────────────
TMP_WORKDIR = "/tmp/stibo_workdir_astec"
os.environ["LAMBDA_TMP_DIR"] = TMP_WORKDIR

# ── ETL modules ──────────────────────────────────────────────────────────────
# import astec.orderform_main       as orderform_etl        # noqa: E402
#import astec.fob_order_info_main  as fob_order_info_etl   # noqa: E402
import astec.recap_main           as recap_etl            # noqa: E402
# import astec.pricelist_main       as pricelist_etl        # noqa: E402
...
log = logging.getLogger("lambda_astec")
...
def _astec_default_metadata(dirs: dict) -> dict:
    """
...
def lambda_handler(event, context, auditor: AuditLogger = None):
    ...
    if _direct_invocation:
        ...
        auditor.set_router_context(
            ...
            detected_brand="astec", routing_decision="astec lambda_handler invoked directly",
        )
    ...
    meta = _astec_default_metadata(dirs)
    log.info("[Astec] Using metadata: %s", meta)
    ...
    log.info("Running Astec ETL → %s ...", etl_module.__name__)
